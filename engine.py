...
class ResultingHTML(object):
	...
	def return_text(self):
		#this will return the entire html text
		pass
...
def evaluate_node(node, context):
	if node['type'] == 'TEXT':
		return my_HTML.update(node['value'])
	if node['type'] == 'VARIABLE':
		var_value = context.fetch(node['value'])
		return my_HTML.update(var_value)
...
outter_context = Scope(None, {'name': 'Leta', 'to_do': ['Monday', 'Tuesday']})
